...
export default function ProjectCaseStudy() {
  ...
  const dc = project.detailedContent || {}
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-background text-foreground selection:bg-brand-500/30">
      ...
      <Link 
        href="/"
        className="fixed top-8 right-8 z-[100] w-14 h-14 bg-white dark:bg-zinc-900 rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all duration-500 group border border-black/5 dark:border-white/10"
      >
         <Home className="text-[#1a0a0a] dark:text-white" size={24} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 md:gap-24 items-start">
          <div className="lg:col-span-5 relative lg:sticky lg:top-32">
             <SectionHeader 
               ...
               icon={Target}
             />
             <p className="text-xl md:text-2xl text-muted-foreground font-inter italic leading-relaxed mt-12 opacity-80">
          ...
          <div className="lg:col-span-7 space-y-16">
            <GlassCard delay={0.2}>
              <h4 className="text-xl font-bold uppercase tracking-widest text-foreground mb-6 flex items-center gap-4">
                <span className="w-8 h-8 rounded-lg bg-brand-500/10 flex items-center justify-center text-brand-500 text-xs">01</span>
                Problem Identification
              </h4>
              <p className="text-lg text-muted-foreground leading-relaxed font-inter">
                {dc.problemStatement || "Identified through gap analysis in existing educational tools where accessibility was often treated as an afterthought."}
              </p>
            </GlassCard>

            <GlassCard delay={0.4}>
              <h4 className="text-xl font-bold uppercase tracking-widest text-foreground mb-6 flex items-center gap-4">
                <span className="w-8 h-8 rounded-lg bg-brand-500/10 flex items-center justify-center text-brand-500 text-xs">02</span>
                The Solution Hypothesis
              </h4>
              <p className="text-lg text-muted-foreground leading-relaxed font-inter">
                {dc.solution || "A multimodal transformation engine that leverages advanced AI to normalize information across disparate media formats."}
              </p>
            </GlassCard>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               {project.features.map((feature, i) => (
                 <motion.div 
                   ...
                   className="group relative p-8 bg-white/40 dark:bg-black/40 backdrop-blur-3xl border border-white/20 dark:border-white/5 rounded-[3rem] shadow-2xl transition-all duration-500 hover:-translate-y-2"
                 >
                   <div className="w-14 h-14 rounded-2xl bg-brand-500/10 flex items-center justify-center text-brand-500 mb-6 group-hover:bg-brand-500 group-hover:text-white transition-all duration-500">
                     <CheckCircle2 size={24} />
                   </div>
                   <h5 className="text-[13px] font-black uppercase tracking-[0.2em] text-foreground mb-4">{feature.split('&')[0]}</h5>
                   <p className="text-[11px] font-medium uppercase tracking-widest text-[#F59E9E] opacity-60 leading-relaxed">Integrated Protocol // V1.0</p>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  {(research.methods.length > 0 ? research.methods : ["User Surveys", "Hotjar Analysis", "A/B Testing", "Focus Groups"]).map((method, i) => (
                    <motion.div 
                      ...
                      className="group relative h-48 bg-[#ffffff] dark:bg-black/40 backdrop-blur-3xl border border-black/10 dark:border-white/5 rounded-[2.5rem] p-8 flex flex-col justify-between transition-all duration-500 hover:border-brand-500/40 hover:shadow-[0_40px_80px_rgba(236,72,153,0.12)] shadow-[0_20px_50px_rgba(0,0,0,0.06)] overflow-hidden"
                    >
        ...
        <div className="flex flex-col lg:flex-row gap-24 items-center">
           <div className="w-full lg:w-1/2">
             <SectionHeader 
                ...
                icon={Palette}
              />
              <p className="text-lg md:text-xl text-muted-foreground font-inter leading-relaxed mt-12 mb-16">
                 {dc.design || "The design system was meticulously crafted to balance professional utility with high-end aesthetic pleasure, utilizing glassmorphism and tactile physics."}
              </p>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
              <div>
                ...
                >
                  ...
                  <p className="text-xl opacity-60 leading-relaxed font-inter mb-12">
                     {dc.coding || "Architected with a modular, scalable approach, ensuring seamless performance and maintainable logic across multimodal data streams."}
                  </p>
                </motion.div>
...
function GlassCard({ children, delay = 0 }: { children: any, delay?: number }) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ delay }}
      className="p-10 md:p-14 bg-white/10 dark:bg-black/20 backdrop-blur-3xl border border-white/20 dark:border-white/5 rounded-[3rem] shadow-2xl"
    >
      {children}
    </motion.div>
  )
