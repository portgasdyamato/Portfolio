...
import { Menu, X, Info, Sparkles, Wand, Code2, ArrowUpRight, LayoutGrid } from "lucide-react"
  ...
  return (
    <>
      ...
      <div className="fixed top-8 left-0 right-0 z-[1000] flex justify-center px-6 pointer-events-none">
        ...
        >
          <div
            className="flex items-center justify-between gap-3 sm:gap-6 px-4 py-2.5 sm:px-10 sm:py-5 pr-2.5 sm:pr-2.5 rounded-full border border-black/[0.08] bg-white/80 backdrop-blur-3xl shadow-[0_1px_2px_rgba(0,0,0,0.05),0_12px_24px_-12px_rgba(0,0,0,0.15)] transition-transform duration-500 scale-100"
          >
            {/* Logo */}
            <button
            ...
            <div className="flex items-center gap-4 pl-6 border-l border-black/5 shrink-0">
              ...
              {/* Custom Scrapbook Redirect Button */}
              <motion.button
                whileHover={{ scale: 1.05, rotate: 90 }}
                whileTap={{ scale: 0.95 }}
                onClick={() => handleNavClick("scrapbook", "/#scrapbook")}
                className="w-10 h-10 rounded-full flex items-center justify-center bg-[#1a0a0a] text-white hover:bg-black transition-all shadow-[0_4px_12px_rgba(0,0,0,0.1)] shrink-0"
              >
                {/* Custom Four-Circle Grid SVG to match user image */}
